...
uniform vec4 rotations[10]; // X, Y, Z in radians; the fourth value is reserved
...
vec3 localPoint(vec3 p, vec4 o, vec4 sp, vec4 rot) {
...
vec3 worldVector(vec3 v, vec4 rot) {
  float cx = cos(rot.x), sx = sin(rot.x), cy = cos(rot.y), sy = sin(rot.y), cz = cos(rot.z), sz = sin(rot.z);
...
vec4 shapeField(vec4 o, vec4 sp, vec4 rot, vec3 p) {
  float type = o.z, stretch = max(.05, sp.x), scale = min(1., stretch); vec3 q = localPoint(p, o, sp, rot); float zz = q.z / stretch; vec4 t;
  ...
  vec3 g = worldVector(vec3(t.y, t.z, t.w / stretch), rot); return vec4(t.x * scale, g * scale);
}
vec4 field(vec3 p) {
  ...
  for (int i = 0; i < 10; i++) { if (i >= shapeCount) break; vec4 o = shapes[i], sp = specs[i], b = shapeField(o, sp, rotations[i], p); f = sp.w > .5 ? smax(f, -b, sp.y) : smin(f, b, sp.y); }
  return f;
}
...
vec3 surfaceColor(vec3 p, vec3 n) {
  float K = gaussian(p, n); vec3 gray = vec3(.57, .60, .58), orange = vec3(1., .46, .12), blue = vec3(.07, .59, .96); float weight = 1. - exp(-abs(K) * 23.); vec3 c = mix(gray, K > 0. ? orange : blue, weight);
  vec3 w = pow(abs(n), vec3(6.)); w /= max(dot(w, vec3(1.)), .0001); float pattern = tiles(p.yz) * w.x + tiles(p.xz) * w.y + tiles(p.xy) * w.z; c *= .94 + pattern * .16;
  // Tint pixels close to each editable primitive's own implicit surface.
  if (mode == 0) for (int i = 0; i < 10; i++) { if (i >= shapeCount) break; vec4 b = shapeField(shapes[i], specs[i], rotations[i], p); float amount = (1. - smoothstep(.08, max(.7, shapes[i].w * .7), abs(b.x))) * .72; c = mix(c, colors[i].rgb, amount); }
...
void main() { vec3 f = normalize(target - camera), r = normalize(cross(f, cameraUp)), u = cross(r, f), ray = normalize(f + (uv.x * aspect.x * r + uv.y * u) * .62), sky = mix(vec3(.69, .77, .82), vec3(.84, .87, .88), .5 + .5 * ray.z); float t = .03; bool hit = false; vec3 p;
  for (int i = 0; i < 150; i++) { p = camera + ray * t; float d = abs(field(p).x); if (d < .006) { hit = true; break; } t += max(.008, d * .65); if (t > 110.) break; }
  if (!hit) { gl_FragColor = vec4(sky, 1.); return; } vec3 n = normal(p), c = surfaceColor(p, n); float light = .67 + .33 * abs(dot(n, normalize(vec3(-.4, -.6, 1.)))); c *= light; c = mix(c, sky, 1. - exp(-t * .003)); gl_FragColor = vec4(c, 1.);
}`;
