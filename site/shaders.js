...
uniform int colorView;uniform float textureDensity;uniform float textureContrast;uniform float drawDistance;uniform float arrowScale;
uniform vec3 player;
...
vec4 shapeField(vec4 o, vec4 sp, vec4 ra, vec4 rb, vec3 p) {
  float type = o.z, stretch = max(.05, sp.x), scale = min(1., stretch); vec3 q = localPoint(p, o, sp, ra, rb); float zz = q.z / stretch, round = min(max(0.,sp.y)*.5,o.w*.45); vec4 t;
  ...
  else if (type > 4.5 && type < 5.5) t = roundedBox(vec3(q.xy, zz), vec3(o.w * .55-round), round);
  else if (type > 6.5) { t = cappedCylinder(vec3(q.xy, zz), o.w-round, o.w*.62-round); t.x-=round; }
...
vec4 baseField(vec3 p) {
  ...
  return vec4(p.z,0.,0.,1.);
}
vec4 field(vec3 p){vec4 f=baseField(p);
...
float tiles(vec2 p) { vec2 q = abs(fract(p * textureDensity) - .5); float sd = length(max(q - vec2(.22), 0.)) - .105; float aa=.018;
#ifdef GL_OES_standard_derivatives
 aa=max(aa,min(.35,fwidth(sd)));
#endif
 return 1. - smoothstep(-aa, aa, sd); }
float arrow(vec3 p) { vec3 d = (p - player) / arrowScale, rt = normalize(cross(forward, up)); float x = dot(d, rt), y = dot(d, forward), z = abs(dot(d, up)); float shaft = max(abs(x) - .09, abs(y + .06) - .22), head = max(abs(x) * .95 + y * .65 - .26, max(-y + .04, y - .42)); return (1. - smoothstep(-.016, .016, min(shaft, head))) * (1. - smoothstep(.07, .15, z)); }
...
vec3 materialColor(vec3 p) {
  vec3 gray = vec3(.57, .60, .58), material = gray; vec4 f = baseField(p);
  ...
  for (int i = 0; i < 10; i++) { if (i >= shapeCount) break; vec4 o = shapes[i], sp = specs[i], b = shapeField(o, sp, rotationA[i], rotationB[i], p);
    ...
    else { float own = (1.-smoothstep(-.035, .035, b.x - f.x)); material = mix(material, colors[i].rgb, own * colors[i].a); f = smin(f, b, sp.y); }
  }
  return material;
}
// Curvature and materials are separate channels: never blend custom tint into K.
vec3 shade(vec3 p,vec3 n,float K,vec3 material){vec3 gray=vec3(.57,.60,.58);float weight=1.-exp(-abs(K)*23.);vec3 curved=mix(gray,K>0.?vec3(1.,.46,.12):vec3(.07,.59,.96),weight);vec3 c=curved;if(colorView==1)c=material;else if(colorView==2&&abs(K)<.0001)c=material;vec3 w=pow(abs(n),vec3(6.));w/=max(dot(w,vec3(1.)),.0001);float pattern=tiles(p.yz)*w.x+tiles(p.xz)*w.y+tiles(p.xy)*w.z;c*=1.+(pattern-.375)*textureContrast;return mix(c,vec3(.83,.025,.035),arrow(p));}
vec3 surfaceColor(vec3 p,vec3 n){return shade(p,n,gaussian(p,n),materialColor(p));}

`;
export const intrinsicVertex =
  `precision highp float;attribute vec2 screen;attribute vec3 point;attribute vec3 surfaceNormal;attribute float curvatureValue;attribute float owner;uniform vec4 colors[10];uniform vec2 aspect;uniform float zoom;uniform vec2 viewOffset;uniform vec2 viewRotation;varying vec3 pos;varying vec3 surfN;varying float surfK;varying vec3 material;void main(){pos=point;surfN=surfaceNormal;surfK=curvatureValue;material=vec3(.57,.60,.58);for(int i=0;i<10;i++){if(abs(owner-float(i))<.5&&colors[i].a>.5)material=colors[i].rgb;}vec2 d=screen-viewOffset;d=vec2(viewRotation.x*d.x-viewRotation.y*d.y,viewRotation.y*d.x+viewRotation.x*d.y);gl_Position=vec4(d/aspect/zoom,0.,1.);}`;
export const intrinsicFragment = common +
  `varying vec3 pos;varying vec3 surfN;varying float surfK;varying vec3 material;void main(){gl_FragColor=vec4(shade(pos,normalize(surfN),surfK,material),1.);}`;
export const quadVertex =
  `attribute vec2 a; varying vec2 uv; void main() { uv = a; gl_Position = vec4(a, 0., 1.); }`;
...
void main() { vec3 f = normalize(target - camera), r = normalize(cross(f, cameraUp)), u = cross(r, f), ray = normalize(f + (uv.x * aspect.x * r + uv.y * u) * .62), sky = mix(vec3(.69, .77, .82), vec3(.84, .87, .88), .5 + .5 * ray.z); float t = .03; bool hit = false; vec3 p;
  for (int i = 0; i < 120; i++) { p = camera + ray * t; float d = abs(field(p).x); if (d < .006) { hit = true; break; } t += max(.008, d * .65); if (t > drawDistance) break; }
