...
export const SHAPE_TYPES = ['handle', 'bump', 'bowl', 'ring', 'plane', 'cube', 'sphere',
  'cylinder'
];
...
function torus(x, y, z, R, r, vertical) {
  const radial = Math.max(vertical ? Math.hypot(x, z) : Math.hypot(x, y), 1e-9),
    a = radial - R,
    b = vertical ? y : z,
...
function cappedCylinder(x, y, z, r, h) {
  const radial = Math.max(Math.hypot(x, y), 1e-9),
    a = radial - r,
    b = Math.abs(z) - h,
    oa = Math.max(a, 0),
    ob = Math.max(b, 0),
...
function rotations(o) {
  return [o.rotationX || 0, o.rotationY || 0, o.rotationZ ?? o.rotation ?? 0];
}

function localPoint(x, y, z, o) {
  const [rx, ry, rz] = rotations(o), cx = Math.cos(rx), sx = Math.sin(rx), cy = Math.cos(ry), sy =
    Math.sin(ry), cz = Math.cos(rz), sz = Math.sin(rz);
  const dx = x - o.x,
    dy = y - o.y,
    dz = z - (o.z || 0);
  const x1 = dx * cz + dy * sz,
    y1 = -dx * sz + dy * cz;
  const x2 = x1 * cy - dz * sy,
    z2 = x1 * sy + dz * cy;
...
export function operationOf(o) {
  return o.operation || (o.type === 'bowl' ? 'negative' : 'additive');
}
...
export function colorOf(o) {
  return operationOf(o) === 'additive' && isCustomColor(o.color) ? o.color : BASE_COLOR;
}
export function hasCustomMaterial(o) {
  return operationOf(o) === 'additive' && isCustomColor(o.color);
}

function shapeField(o, x, y, z) {
  const [rx, ry, rz] = localPoint(x, y, z, o), stretch = Math.max(.05, o.strength), scale = Math
    .min(1, stretch), zz = rz / stretch;
  const round = Math.min(Math.max(0, o.blend) * .5, o.size * .45);
  let t;
  ...
  else if (o.type === 'cube') t = roundedBox(rx, ry, zz, o.size * .55 - round, o.size * .55 - round, o.size * .55 - round,
    round);
  else if (o.type === 'cylinder') {
    t = cappedCylinder(rx, ry, zz, o.size - round, o.size * .62 - round);
    t[0] -= round;
  } else {
...
export function project(p, c, iterations = 4) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    if (gg < 1e-24) break;
    const t = clamp(f[0] / gg, -1e12, 1e12);
    q = [q[0] - t * f[1], q[1] - t * f[2], q[2] - t * f[3]];
  }
  // Editing can remove the surface directly under the player. At CSG ridges,
  // Newton projection may cycle; find a nearby sign bracket only for explicit
  // spawn/edit projection, never for ordinary movement steps.
  if (iterations >= 20 && Math.abs(field(q, c)[0]) > 1e-5) {
    const origin = p.slice(),
      f0 = field(origin, c)[0],
      unit = Math.max(.05, Math.min(c.radius || 1, ...c.shapes.map(o => o.size)) * .04);
    outer: for (let shell = 0; shell < 22; shell++) {
      const radius = unit * 2 ** shell;
      for (let dx = -1; dx <= 1; dx++)
        for (let dy = -1; dy <= 1; dy++)
          for (let dz = -1; dz <= 1; dz++) {
            if (!dx && !dy && !dz) continue;
            const direction = norm([dx, dy, dz]);
            let hi = add(origin, mul(direction, radius)),
              fh = field(hi, c)[0];
            if (f0 * fh > 0) continue;
            let lo = origin.slice(),
              fl = f0;
            for (let j = 0; j < 45; j++) {
              const mid = mul(add(lo, hi), .5),
                fm = field(mid, c)[0];
              if (fl * fm <= 0) hi = mid;
              else {
                lo = mid;
...
export function transport(v, n0, n1) {
  return norm(sub(v, mul(add(n0, n1), dot(v, n1) / Math.max(.01, 1 + dot(n0, n1)))));
}
export function step(p, v, h, c) {
  const n0 = normal(p, c),
    q = project(add(p, mul(v, h)), c, 3),
    n1 = normal(q, c);
  if (!q.every(Number.isFinite) || Math.abs(field(q, c)[0]) > .001) return {
    p: [...p],
    v: [...v],
    n: n0
  };
...
export function spawn(c, onShape = false, index = 0) {
  let p, v;
  if (onShape && c.shapes[index]) {
    const o = c.shapes[index],
      local = [0, 0, o.size * Math.max(1, o.strength) + 1];
    p = add([o.x, o.y, o.z || 0], worldVector(local, o));
    v = worldVector([0, 1, 0], o);
  } else if (c.mode === 'cylinder') {
    p = [0, 0, c.radius];
    v = [1, 0, 0];
    ...
    const o = c.shapes[index] || c.shapes[0] || {
      x: 0,
      y: 0,
      z: 0,
      size: 3.4,
    ...
    if (onShape) {
      p = [o.x, o.y, (o.z || 0) + o.size * Math.max(1, o.strength) + 1];
      v = [0, 1, 0];
    } else {
  ...
  if (Math.hypot(...v) < 1e-8) v = cross(n, Math.abs(n[2]) < .9 ? [0, 0, 1] : [0, 1, 0]);
  v = norm(v);
  return {
    p,
    v,
